...
// Lista categorie speciali per il modal — da SpecialFolderMap + SuffixConfig
const SPECIAL_CATEGORY_LIST = Object.entries(SpecialFolderMap).map(([folderName, suffixId]) => {
  const cfg  = SuffixConfig[suffixId];
  const slug = folderName.replace(/\s+/g, '-');
  return { suffixId, slug, label: cfg?.label ?? folderName, color: cfg?.color ?? 'bg-purple-600' };
});
...
const FolderCard = ({ folderKey, mobs, trackedMobs, isOpen, onToggle }) => {
  ...
  return (
    <div onClick={onToggle} className={`group cursor-pointer border-4 transition-all select-none
      ${isOpen
      ...
      <div className="aspect-square p-2 flex items-center justify-center bg-[#181818] relative overflow-hidden">
        {firstMob && <img src={firstMob.image} alt={label} draggable={false} className="max-w-full max-h-full object-contain pixelated opacity-60 group-hover:opacity-80 transition-opacity select-none" />}
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40">
          <span className={`text-2xl leading-none ${isSpecial ? 'text-purple-300' : 'text-blue-300'}`}>{isOpen ? '▼' : '▶'}</span>
        </div>
      ...
      <div className={`p-1 border-t-4 ${isOpen
        ...
        <p className={`text-[10px] leading-tight uppercase truncate px-1 text-center ${isSpecial ? 'text-purple-200' : 'text-blue-200'}`}>{label}</p>
        <div className="mt-1 h-1 bg-stone-900 mx-1">
          <div className={`h-full transition-all duration-500 ${pct === 100 ? 'bg-green-400' : isSpecial ? 'bg-purple-500' : 'bg-blue-500'}`} style={{ width: `${pct}%` }} />
        </div>
      </div>
    </div>
  );
...
const MobTracker = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const data = Object.keys(modules).map(path => {
        ...
        return { ...parseFileName(fileName, path), image: path.replace('/public', ''), fileName: uniqueKey, displayFileName: fileName, folder, specialSuffixId };
  ...
  const undo = () => {
    ...
    setTrackedMobs(prev.trackedMobs); setCapturedMobs(prev.capturedMobs);
  };
  ...
  const redo = () => {
    ...
    setTrackedMobs(next.trackedMobs); setCapturedMobs(next.capturedMobs);
  };
  ...
  useEffect(() => {
    const handleKey = (e) => {
      ...
      if (e.key === 'z' && (e.ctrlKey || e.metaKey) && !e.shiftKey && !['INPUT','SELECT','TEXTAREA'].includes(tag)) { e.preventDefault(); undo(); return; }
      if ((e.key === 'y' && (e.ctrlKey || e.metaKey) || e.key === 'z' && (e.ctrlKey || e.metaKey) && e.shiftKey) && !['INPUT','SELECT','TEXTAREA'].includes(tag)) { e.preventDefault(); redo(); return; }
      if (e.key === ' ' && !['INPUT','SELECT','TEXTAREA'].includes(tag)) { e.preventDefault(); searchRef.current?.focus(); }
    };
  ...
  const folderList = useMemo(() => {
    ...
    return Array.from(set).sort().map(f => {
      ...
      return { id: `${FOLDER_FILTER_PREFIX}${f}`, variantsId: `variants:${f}`, label: f.charAt(0).toUpperCase() + f.slice(1), linkedComplexId: linked?.id ?? null };
    });
  }, [allMobs]);
  ...
  const specialBtns = useMemo(() => {
    ...
    Object.entries(SpecialFolderMap).forEach(([folderName, suffixId]) => {
      ...
      map.set(folderKey, { folderKey, suffixId, label: config.label ?? suffixId });
    });
    allMobs.forEach(m => {
  ...
  const sortMobs = (mobs, sortKey, tracked) => {
    ...
    switch (sortKey) {
      ...
      case 'untracked':  return arr.sort((a, b) => { const d = (tracked[a.fileName]?1:0)-(tracked[b.fileName]?1:0); return d!==0?d:a.name.localeCompare(b.name); });
      case 'tracked':    return arr.sort((a, b) => { const d = (tracked[b.fileName]?1:0)-(tracked[a.fileName]?1:0); return d!==0?d:a.name.localeCompare(b.name); });
      case 'biome': {
        const getLabel = (mob) => { const config = ComplexConfig.find(c => c.id === mob.complexId); return config?.useVillagerIcons && mob.num1 != null ? (VillagerBiomes?.[mob.num1]?.label ?? '') : ''; };
        return arr.sort((a, b) => { const ba=getLabel(a),bb=getLabel(b); if(ba!==bb){if(!ba)return 1;if(!bb)return -1;return ba.localeCompare(bb);} return a.name.localeCompare(b.name); });
      }
      case 'job': {
        const getLabel = (mob) => { const config = ComplexConfig.find(c => c.id === mob.complexId); return config?.useVillagerIcons && mob.num2 != null ? (VillagerJobs?.[mob.num2]?.label ?? '') : ''; };
        return arr.sort((a, b) => { const ja=getLabel(a),jb=getLabel(b); if(ja!==jb){if(!ja)return 1;if(!jb)return -1;return ja.localeCompare(jb);} return a.name.localeCompare(b.name); });
      }
      default: return arr;
    }
  };
  ...
  const displayedMobs = useMemo(() => {
    ...
    const filtered = allMobs.filter(mob => {
      if (searchQuery) { const words = searchQuery.toLowerCase().trim().split(/\s+/); if (!words.every(w => getMobSearchString(mob).includes(w))) return false; }
      if (selectedFolder !== 'all') {
        if (selectedSpecialSuffixId) { if (mob.folder !== selectedFolder && !mob.activeSuffixes.includes(selectedSpecialSuffixId) && mob.specialSuffixId !== selectedSpecialSuffixId) return false; }
        else { if (mob.folder !== selectedFolder) return false; }
      }
      ...
      if (mob.folder.startsWith('special:') && mob.specialSuffixId) {
        ...
        if (!specialVarActive) { if ((mob.num1&&mob.num1>1)||(mob.num2&&mob.num2>1)||(mob.num3&&mob.num3>1)) return false; }
      }
      if (mob.folder !== 'root' && !mob.folder.startsWith('special:')) {
        const variantsActive = filters[`variants:${mob.folder}`] !== false;
        if (!variantsActive) { if ((mob.num1&&mob.num1>1)||(mob.num2&&mob.num2>1)||(mob.num3&&mob.num3>1)) return false; return mob.activeSuffixes.length === 0; }
      }
      ...
      if (variantMode === 'none') { if (mob.num1>1||(mob.num2&&mob.num2>1)||(mob.num3&&mob.num3>1)) return false; }
      else if (variantMode === 'main') { if ((mob.num2&&mob.num2>1)||(mob.num3&&mob.num3>1)) return false; }
      return true;
    });
  ...
  const groupedMobs = useMemo(() => {
    ...
    const map = new Map();
    displayedMobs.forEach(mob => { if (!map.has(mob.folder)) map.set(mob.folder, []); map.get(mob.folder).push(mob); });
    return Array.from(map.entries()).sort(([a],[b]) => {
      if (a==='root') return -1; if (b==='root') return 1;
      const aS=a.startsWith('special:'), bS=b.startsWith('special:');
  ...
  const gridItems = useMemo(() => {
    ...
    for (const [folderKey, mobs] of groupedMobs) {
      ...
      if (openFolders.has(folderKey)) { for (const mob of mobs) items.push({ type: 'mob', mob }); }
    }
    return items;
  }, [groupedMobs, openFolders]);

  const toggleFolder = (folderKey) => {
    setOpenFolders(prev => { const next = new Set(prev); if (next.has(folderKey)) next.delete(folderKey); else next.add(folderKey); return next; });
  };
  ...
  const getMobName = (id) => { const mob = allMobs.find(m => m.fileName === id); return mob ? mob.name : id; };
  const isAddAction = (id) => { if (captureMode) { const wT=!!trackedMobs[id], wC=!!capturedMobs[id]; return !wT&&!wC; } return !capturedMobs[id]; };

  const executeToggle = (id) => {
    commitAction(() => {
      if (captureMode) {
        const wasTracked=!!trackedMobs[id], wasCaptured=!!capturedMobs[id];
        if (!wasTracked&&!wasCaptured) { setTrackedMobs(p=>({...p,[id]:true})); setCapturedMobs(p=>{const n={...p};delete n[id];return n;}); }
        else if (wasTracked&&!wasCaptured) { setCapturedMobs(p=>({...p,[id]:true})); }
        else { setTrackedMobs(p=>{const n={...p};delete n[id];return n;}); setCapturedMobs(p=>{const n={...p};delete n[id];return n;}); }
      } else {
        if (!!capturedMobs[id]) { setTrackedMobs(p=>{const n={...p};delete n[id];return n;}); setCapturedMobs(p=>{const n={...p};delete n[id];return n;}); }
        else { setTrackedMobs(p=>({...p,[id]:true})); setCapturedMobs(p=>({...p,[id]:true})); }
      }
    });
  };
  ...
  const toggleMob = (id) => {
    ...
    if (adding && confirmAdd)     { setPendingAction({ type: 'add',    label: getMobName(id), execute: () => executeToggle(id) }); return; }
    if (removing && confirmRemove){ setPendingAction({ type: 'remove', label: getMobName(id), execute: () => executeToggle(id) }); return; }
    executeToggle(id);
  };
  ...
  const executeApplyDrag = (ids) => {
    commitAction(() => {
      if (captureMode) {
        const allTracked=[ ...ids].every(id=>!!trackedMobs[id]), allCaptured=[...ids].every(id=>!!capturedMobs[id]);
        if (allCaptured) { setTrackedMobs(p=>{const n={...p};ids.forEach(id=>delete n[id]);return n;}); setCapturedMobs(p=>{const n={...p};ids.forEach(id=>delete n[id]);return n;}); }
        else if (allTracked) { setCapturedMobs(p=>{const n={...p};ids.forEach(id=>{n[id]=true;});return n;}); }
        else { setTrackedMobs(p=>{const n={...p};ids.forEach(id=>{if(!n[id])n[id]=true;});return n;}); }
      } else {
        const allCaptured=[...ids].every(id=>!!capturedMobs[id]);
        if (allCaptured) { setTrackedMobs(p=>{const n={...p};ids.forEach(id=>delete n[id]);return n;}); setCapturedMobs(p=>{const n={...p};ids.forEach(id=>delete n[id]);return n;}); }
        else { setTrackedMobs(p=>{const n={...p};ids.forEach(id=>{n[id]=true;});return n;}); setCapturedMobs(p=>{const n={...p};ids.forEach(id=>{n[id]=true;});return n;}); }
      }
    });
  };
  ...
  const applyDragSelection = (ids) => {
    ...
    const allCaptured=[...ids].every(id=>!!capturedMobs[id]);
    if (allCaptured && confirmRemove) { setPendingAction({ type:'remove', label:`${ids.size} mob`, execute:()=>executeApplyDrag(ids) }); return; }
    if (!allCaptured && confirmAdd)   { setPendingAction({ type:'add',    label:`${ids.size} mob`, execute:()=>executeApplyDrag(ids) }); return; }
    executeApplyDrag(ids);
  };
  applyDragRef.current = applyDragSelection;

  const gridRef=useRef(null), selBoxRef=useRef(null), isDragging=useRef(false), dragStart=useRef(null);
  const dragStartedOnCard=useRef(false), selRectRef=useRef(null), selectedMobsRef=useRef(new Set());
  ...
  const handleGridMouseDown = (e) => {
    ...
    dragStartedOnCard.current=onCard; isDragging.current=true; dragStart.current={x:e.pageX,y:e.pageY};
    selectedMobsRef.current=new Set();
    if (!onCard) e.preventDefault();
  };

  useEffect(() => {
    if (!selectionMode) { setSelectedMobs(new Set()); if (selBoxRef.current) selBoxRef.current.style.display='none'; return; }
    const onMouseMove=(e)=>{
      if (!isDragging.current||!dragStart.current) return;
      const px=e.pageX,py=e.pageY,dx=Math.abs(px-dragStart.current.x),dy=Math.abs(py-dragStart.current.y);
      if (dx<DRAG_THRESHOLD&&dy<DRAG_THRESHOLD) return;
      dragStartedOnCard.current=false;
      const x=Math.min(px,dragStart.current.x),y=Math.min(py,dragStart.current.y),w=Math.abs(px-dragStart.current.x),h=Math.abs(py-dragStart.current.y);
      selRectRef.current={x,y,w,h};
      if (selBoxRef.current) { selBoxRef.current.style.display='block'; selBoxRef.current.style.left=`${x}px`; selBoxRef.current.style.top=`${y}px`; selBoxRef.current.style.width=`${w}px`; selBoxRef.current.style.height=`${h}px`; }
      const scrollX=window.scrollX,scrollY=window.scrollY,sel=new Set();
      getCardEls().forEach(el=>{const r=el.getBoundingClientRect(),ex=r.left+scrollX,ey=r.top+scrollY,ew=r.width,eh=r.height;if(ex<x+w&&ex+ew>x&&ey<y+h&&ey+eh>y){sel.add(el.dataset.mobId);el.dataset.selected='true';}else{delete el.dataset.selected;}});
      selectedMobsRef.current=sel;
    };
    const onMouseUp=()=>{
      if (!isDragging.current) return;
      const wasRealDrag=!!selRectRef.current;
      isDragging.current=false; dragStart.current=null; dragStartedOnCard.current=false; selRectRef.current=null;
      if (selBoxRef.current) selBoxRef.current.style.display='none';
      getCardEls().forEach(el=>delete el.dataset.selected);
      if (wasRealDrag&&selectedMobsRef.current.size>0) { const ids=new Set(selectedMobsRef.current); selectedMobsRef.current=new Set(); applyDragRef.current(ids); }
      else { selectedMobsRef.current=new Set(); }
      setSelectedMobs(new Set());
    };
    const onKeyDown=(e)=>{
      if (e.key==='Escape'&&isDragging.current) { isDragging.current=false; dragStart.current=null; dragStartedOnCard.current=false; selRectRef.current=null; if(selBoxRef.current)selBoxRef.current.style.display='none'; getCardEls().forEach(el=>delete el.dataset.selected); selectedMobsRef.current=new Set(); setSelectedMobs(new Set()); }
    };
    window.addEventListener('mousemove',onMouseMove,{passive:true});
    window.addEventListener('mouseup',onMouseUp);
    window.addEventListener('keydown',onKeyDown);
    return ()=>{ window.removeEventListener('mousemove',onMouseMove); window.removeEventListener('mouseup',onMouseUp); window.removeEventListener('keydown',onKeyDown); };
  }, [selectionMode]);

  const currentSortLabel = SORT_OPTIONS.find(o => o.value === sortBy)?.label ?? '↑ A→Z';

  return (
    <div className="min-h-screen bg-[#111] text-stone-100 flex flex-col" onMouseDown={handleGridMouseDown} style={{ cursor: selectionMode?'crosshair':undefined, position:'relative' }}>
      <style>{`
      ...
      {pendingAction && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-stone-900 border-4 border-stone-600 shadow-2xl p-8 max-w-sm w-full mx-4">
            <p className="text-xl uppercase text-stone-200 mb-2">{pendingAction.type==='add'?'➕ Conferma aggiunta':'🗑 Conferma rimozione'}</p>
            <p className="text-stone-400 text-sm uppercase mb-6">{pendingAction.type==='add'?`Aggiungere ${pendingAction.label}?`:`Rimuovere ${pendingAction.label}?`}</p>
            <div className="flex gap-3">
              <button onClick={()=>{pendingAction.execute();setPendingAction(null);}} className={`flex-1 py-3 border-b-4 border-black uppercase text-sm transition-transform active:translate-y-1 active:border-b-0 ${pendingAction.type==='add'?'bg-green-700 hover:bg-green-600':'bg-red-800 hover:bg-red-700'}`}>Conferma</button>
              <button onClick={()=>setPendingAction(null)} className="flex-1 py-3 bg-stone-700 hover:bg-stone-600 border-b-4 border-black uppercase text-sm transition-transform active:translate-y-1 active:border-b-0">Annulla</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showSettings && (
        <Settings variantMode={variantMode} setVariantMode={setVariantMode} filters={filters} toggleFilter={(id)=>setFilters(p=>({...p,[id]:!p[id]}))} setFilters={setFilters}
          showAllFish={showAllFish} setShowAllFish={setShowAllFish} folderList={folderList}
          resetAll={()=>{if(confirm('Sei sicuro di voler resettare tutti i progressi?')){setTrackedMobs({});setCapturedMobs({});}}}
          onClose={()=>setShowSettings(false)} captureMode={captureMode} setCaptureMode={setCaptureMode}
          selectionMode={selectionMode} setSelectionMode={setSelectionMode} confirmAdd={confirmAdd} setConfirmAdd={setConfirmAdd}
      ...
      {/* ── Modal categorie ── */}
      {showCategories && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={()=>setShowCategories(false)}>
          <div className="bg-stone-900 border-4 border-amber-800 shadow-2xl w-full max-w-2xl flex flex-col" style={{maxHeight:'80vh'}} onClick={e=>e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b-4 border-amber-900 shrink-0">
              ...
              <button onClick={()=>setShowCategories(false)} className="bg-stone-700 hover:bg-stone-600 text-white px-4 py-1.5 text-xs border-b-4 border-black uppercase transition-transform active:translate-y-1 active:border-b-0">Close</button>
            </div>

            <div className="overflow-y-auto flex-1 p-4 space-y-2" style={{scrollbarWidth:'thin',scrollbarColor:'#44403c #0c0c0c'}}>

              {/* ── Categorie speciali ── */}
              <p className="text-[10px] uppercase text-stone-600 tracking-widest px-1 pb-1">Special</p>
              {SPECIAL_CATEGORY_LIST.map(sc => (
                <button key={sc.suffixId}
                  onClick={()=>{ setShowCategories(false); navigate(`/${sc.slug}`); }}
                  className="flex items-center gap-4 w-full p-4 border-2 border-purple-900 hover:border-purple-600 bg-stone-800 hover:bg-stone-700 transition-all group text-left">
                  <div className="w-10 h-10 shrink-0 flex items-center justify-center bg-[#181818] border-2 border-stone-700">
                    <div className={`${sc.color} text-white text-[9px] px-1 py-0.5 border border-stone-900 uppercase leading-none text-center`}>{sc.label}</div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm uppercase text-purple-400">{sc.label}</p>
                    <p className="text-xs text-stone-600 mt-0.5">Special category</p>
                  </div>
                  <span className="text-stone-600 group-hover:text-stone-300 transition-colors text-lg shrink-0">→</span>
                </button>
              ))}

              {/* ── Separatore ── */}
              <div className="border-t-2 border-stone-800 my-2" />
              <p className="text-[10px] uppercase text-stone-600 tracking-widest px-1 pb-1">Standard</p>

              {/* ── Categorie normali ── */}
              {MobCategories.map(cat => {
                const colorMap = {
                  amber:  { border:'border-amber-800',  hover:'hover:border-amber-500',  text:'text-amber-400'  },
                  blue:   { border:'border-blue-800',   hover:'hover:border-blue-500',   text:'text-blue-400'   },
                  green:  { border:'border-green-800',  hover:'hover:border-green-500',  text:'text-green-400'  },
                  red:    { border:'border-red-800',    hover:'hover:border-red-500',    text:'text-red-400'    },
                  purple: { border:'border-purple-800', hover:'hover:border-purple-500', text:'text-purple-400' },
                  cyan:   { border:'border-cyan-800',   hover:'hover:border-cyan-500',   text:'text-cyan-400'   },
                  white:  { border:'border-stone-600',  hover:'hover:border-stone-400',  text:'text-stone-300'  },
                };
                const c = colorMap[cat.color] ?? colorMap.amber;
                return (
                  <button key={cat.id} onClick={()=>{ setShowCategories(false); navigate(`/categories/${cat.id}`); }}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {cat.description && <p className="text-xs text-stone-500 leading-relaxed mt-1 line-clamp-2">{cat.description}</p>}
                    </div>
                    <span className="text-stone-600 group-hover:text-stone-300 transition-colors text-lg shrink-0">→</span>
                  </button>
                );
      ...
      <div className="flex-1 p-4 md:p-6">
        <div className="max-w-[1600px] mx-auto">
          <header className="bg-stone-800 rounded-lg p-6 mb-6 border-4 border-stone-600 shadow-xl">
            <div className="flex flex-col lg:flex-row justify-between items-center gap-6 mb-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-3/4 justify-end items-stretch">
                <button onClick={()=>setGroupByFolder(v=>!v)} title={groupByFolder?'Disattiva raggruppamento':'Raggruppa per cartella'}
                  className={`shrink-0 flex items-center justify-center gap-2 px-3 py-2 border-4 text-sm uppercase transition-colors ${groupByFolder?'bg-blue-800 border-blue-500 text-blue-200 hover:bg-blue-700':'bg-stone-900 border-stone-700 text-stone-500 hover:border-stone-500 hover:text-stone-300'}`}>
                  <span className="text-base leading-none">▤</span>
                  <span className="hidden sm:inline text-xs">Cartelle</span>
                </button>
                <div ref={sortRef} className="relative shrink-0">
                  <button onClick={()=>setSortOpen(v=>!v)} className="w-full h-full bg-black border-4 border-stone-700 px-4 py-2 text-sm uppercase text-stone-300 hover:border-stone-500 hover:text-white transition-colors flex items-center gap-2 whitespace-nowrap">
                    ...
                    <div className="absolute top-full left-0 mt-1 bg-stone-900 border-4 border-stone-600 z-50 min-w-full shadow-2xl">
                      {SORT_OPTIONS.map(opt=>(
                        <button key={opt.value} onClick={()=>{setSortBy(opt.value);setSortOpen(false);}}
                          className={`w-full text-left px-4 py-2 text-sm uppercase whitespace-nowrap transition-colors ${sortBy===opt.value?'bg-green-800 text-green-200':'text-stone-300 hover:bg-stone-700 hover:text-white'}`}>
                          {opt.label}
                        </button>
                      ))}
                    </div>
                ...
                <div className="relative flex-grow max-w-2xl">
                  <input ref={searchRef} type="text" placeholder="Search… (premi Spazio)" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}
                  ...
                  {searchQuery && <button onClick={()=>setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-white text-xl pb-1">X</button>}
                </div>
            ...
            <div className="mb-4 flex flex-wrap gap-2 items-center">
              <button onClick={()=>setSelectedFolder('all')} className={`px-3 py-1 text-sm uppercase border-b-4 transition-all active:translate-y-1 active:border-b-0 ${selectedFolder==='all'?'bg-green-700 border-green-900 text-white':'bg-stone-700 border-stone-900 text-stone-300 hover:bg-stone-600'}`}>Tutti</button>
              {specialBtns.length>0&&(<><span className="text-stone-600 select-none">|</span>
                {specialBtns.map(b=>(
              ...
              {allFolderBtns.length>0&&(<><span className="text-stone-600 select-none">|</span>
                {allFolderBtns.filter(btn=>btn.type!=='fish'||fishVisible).map(btn=>
                  btn.type==='fish'
                    ? <button key={btn.key} onClick={()=>setSelectedFolder(FISH_FOLDER)} className={`px-3 py-1 text-sm uppercase border-b-4 transition-all active:translate-y-1 active:border-b-0 ${selectedFolder===FISH_FOLDER?'bg-green-700 border-green-900 text-white':'bg-stone-700 border-stone-900 text-stone-300 hover:bg-stone-600'}`}>{btn.label}</button>
                    : <button key={btn.key} onClick={()=>setSelectedFolder(btn.key)} className={`px-3 py-1 text-sm uppercase border-b-4 transition-all active:translate-y-1 active:border-b-0 ${selectedFolder===btn.key?'bg-green-700 border-green-900 text-white':'bg-stone-700 border-stone-900 text-stone-300 hover:bg-stone-600'}`}>{btn.label}</button>
                )}</>
              )}
            </div>
              ...
              <div className="h-6 bg-stone-900 border-2 border-stone-700 p-1 overflow-hidden">
                {captureMode ? (
                  <div className="h-full flex">
                    <div className="h-full bg-green-500 transition-all duration-500" style={{width:`${totalDisplayed>0?(totalCaptured/totalDisplayed)*100:0}%`}} />
                    <div className="h-full bg-yellow-500 transition-all duration-500" style={{width:`${totalDisplayed>0?((totalTracked-totalCaptured)/totalDisplayed)*100:0}%`}} />
                  </div>
                ) : <div className="h-full bg-green-500 transition-all duration-500" style={{width:`${totalDisplayed>0?(totalCaptured/totalDisplayed)*100:0}%`}} />}
              </div>
            </div>
          ...
          {!isFishOnly && (
            <div ref={gridRef} className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-3 mb-6" style={{userSelect:selectionMode?'none':undefined}}>
              {!groupByFolder
                ? displayedMobs.map(mob=>(
                    <MobCard key={mob.fileName} mob={mob} isTracked={trackedMobs[mob.fileName]} isCaptured={!!capturedMobs[mob.fileName]}
                      captureMode={captureMode} selectionMode={selectionMode} onToggle={()=>toggleMob(mob.fileName)} data-mob-id={mob.fileName} />
                  ))
                : gridItems.map((item,i)=>
                    item.type==='folder'
                      ? <FolderCard key={`folder_${item.folderKey}`} folderKey={item.folderKey} mobs={item.mobs} trackedMobs={trackedMobs} isOpen={openFolders.has(item.folderKey)} onToggle={()=>toggleFolder(item.folderKey)} />
                      : <MobCard key={item.mob.fileName} mob={item.mob} isTracked={trackedMobs[item.mob.fileName]} isCaptured={!!capturedMobs[item.mob.fileName]}
                          captureMode={captureMode} selectionMode={selectionMode} onToggle={()=>toggleMob(item.mob.fileName)} data-mob-id={item.mob.fileName} />
                  )
              }
              {!groupByFolder&&displayedMobs.length===0&&selectedFolder.startsWith('special:')&&(
                <div className="col-span-full flex flex-col items-center justify-center py-16 text-stone-500">
                  ...
                  <p className="text-lg uppercase">No mobs in <span className="text-purple-400">{selectedFolder.replace('special:','')}</span> yet</p>
          ...
          <div ref={selBoxRef} className="pointer-events-none z-50 border-2 border-green-400 bg-green-400/10" style={{display:'none',position:'absolute'}} />
            ...
            <div className="bg-stone-800 border-4 border-cyan-900 rounded-lg mb-6">
              <div className="w-full flex justify-between items-center px-6 py-4 border-b-4 border-cyan-900">
                <div className="flex items-center gap-3">
                  ...
                  <span className="bg-cyan-900 text-cyan-300 text-sm px-2 py-0.5 border-2 border-cyan-800">{fishTrackedCount} / {fishPool.length}</span>
                  <span className="text-stone-500 text-sm uppercase">{variantMode==='none'?'(1 base)':variantMode==='main'?'(22 named)':showAllFish?'(3072 tutte)':'(22 named)'}</span>
                </div>
              </div>
              <div className="p-4">
