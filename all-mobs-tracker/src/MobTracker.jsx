...
const MobTracker = () => {
  ...
  const [showHorses, setShowHorses] = useState(false); // Stato cavalli
  ...
  const parseFileName = (fileName, path) => {
    ...
    if (path && path.includes('/horses/')) {
      ...
      if (matchH) {
        ...
        const equip = { 
          1: '', 2: 'Saddle', 3: 'Leather Armor', 4: 'Iron Armor', 5: 'Gold Armor', 6: 'Diamond Armor', 
          7: 'Netherite Armor', 8: 'Saddle + Leather', 9: 'Saddle + Iron', 10: 'Saddle + Gold', 
          11: 'Saddle + Diamond', 12: 'Saddle + Netherite' 
        };
  ...
  useEffect(() => {
    ...
    // FILTRO VILLAGER (Nasconde tutto tranne 1.1 adulto se OFF)
    if (!showVillagers) {
      filtered = filtered.filter(m => !m.isVillager || (m.num1 === 1 && m.num2 === 1 && !m.isRealBaby && !m.isBreedBaby));
    }

    // FILTRO HORSES (Nasconde tutto tranne 1.1.1 adulto se OFF)
    if (!showHorses) {
      filtered = filtered.filter(m => !m.isHorse || (m.num1 === 1 && m.num2 === 1 && m.num3 === 1 && !m.isRealBaby && !m.isBreedBaby));
    }
      ...
      <div className="max-w-[1600px] mx-auto">
        <header className="bg-stone-800 rounded-lg p-6 mb-6 border-4 border-stone-600 shadow-xl">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-6">
            ...
            <div className="flex gap-4 w-full md:w-auto items-center">
              <input type="text" placeholder="Cerca..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-[#0a0a0a] border-4 border-stone-700 text-2xl px-3 py-1 outline-none h-[48px]" />
          ...
          <div className="bg-[#1a1a1a] p-4 rounded border-4 border-stone-700 grid grid-cols-2 md:grid-cols-6 gap-4 text-center">
            <div className="col-span-2 md:col-span-1 border-stone-700 md:border-r-2">
               <span className="text-stone-400 uppercase">Visti</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-9 gap-3">
          {displayedMobs.map((mob) => {
            ...
            return (
              <div key={mob.fileName} onClick={() => toggleMob(mob.fileName)} className={`group relative bg-stone-800 border-4 cursor-pointer overflow-hidden ${isTracked ? 'border-green-600' : 'border-stone-700'}`}>
                <div className={`aspect-square p-2 flex items-center justify-center bg-[#181818] ${isTracked ? 'opacity-40' : ''}`}>
                  <img src={mob.image} alt={mob.name} className="max-w-full max-h-full object-contain pixelated" />
                  {isTracked && <div className="absolute inset-0 flex items-center justify-center"><span className="text-green-500 text-7xl">✔</span></div>}
                </div>
                <div className="p-1 text-center border-t-4 border-stone-700 bg-stone-800">
                  <p className="text-[10px] leading-tight text-stone-200 uppercase truncate px-1">{mob.name}</p>
                </div>
              </div>
            );
          })}
